...
let options = { autoAim: { scaleFactor: 2.5, predictMovement: false }, renderPlayers: !0, renderTargets: !0, renderPowerups: !0, scrollZoom: !0, scrollSensitivity: 70, freeCamSensitivity: 12, spectatingPlayer: null, autoShoot: !1, autoAnswer: !0 };
  ...
  room.onMessage("playerPosUpdate", () => {
    if (options.autoShoot) {
      ...
      let players = Array.from(room.state.players)
        .filter(e => e[1].sessionId && e[1] !== localp[1] && !e[1].invulnerable && !lineIsBlocked(new Phaser.Geom.Line(localp[1].x, localp[1].y, e[1].x, e[1].y)));
      if (players.length < 1) {
        let targets = Array.from(room.state.targets)
          .filter(e => !e[1].isHit && !lineIsBlocked(new Phaser.Geom.Line(localp[1].x, localp[1].y, e[1].x, e[1].y)))
