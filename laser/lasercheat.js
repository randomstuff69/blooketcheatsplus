  ...
  function runCheat() {
    ...
    room.onMessage("playerPosUpdate", e => {
      if (options.autoShoot) {
        ...
        if (Date.now() - lastMouseSend > 50) {room.send("playerMouseUpdate", { mouseAngle: playerAngle });lastMouseSend = Date.now();}
        if (Date.now() - lastSend > 50) {room.send("playerMouseClick", null); lastSend = Date.now(); }
      }
    }); run = 1;
  }
